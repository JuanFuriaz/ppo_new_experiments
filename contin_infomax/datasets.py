...
from contin_infomax.utils import RandomErasing


class RacingcarMultiviewDataset(Dataset):

    def __init__(self, datadir, train=True, transform=None, transform2=None, target_transform=None, img_stack=1, cutout=False):
        ...
        self.cutout = cutout
        self.eraser = RandomErasing(1.0, return_resized_erasure=True)
        self.img_stack = img_stack

    def __getitem__(self, index):
        img, target = self.data[index], self.targets[index]

        # apply two independent transforms to img
        if self.transform is not None:
            # img = self.transform(img)
            if self.cutout:
                # img1, img2 = self.eraser(img)
                raise NotImplementedError("Need to revise this implementation to generalized img_stack")
            else:
                img1 = [self.transform(img[i]) for i in range(img.shape[0])]
                img1 = torch.stack(img1).squeeze(1)
                img2 = [self.transform(img[i]) for i in range(img.shape[0])]
                img2 = torch.stack(img2).squeeze(1)

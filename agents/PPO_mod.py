...
class PPO(object):
    def __init__(
        ...
        v2 = v_base(ndim=ndim, action_vec=self.action_vec)
        ...
        if vae:  # Variational Autoenconder
            ...
            self.critic_target = CriticVAE(vae, v2).float().to(device)
            print("\nPPO VAE STARTED\n")
            ...
            self.critic_target = Critic(infomax, v2).float().to(device)
            print("\nPPO INFOMAX STARTED\n")
        elif rnn:
            ...
            self.critic_target = Critic(raw_cnn, v2).float().to(device)
            print("\nPPO RAW-PIXEL STARTED\n")
        # Join optimization of actor and critic
        params_opt = list(self.actor.parameters()) + list(self.critic.parameters()) + list(

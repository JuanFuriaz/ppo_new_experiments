...
class PPO(object):
    def __init__(
        ...
        if vae:  # Variational Autoenconder
            ...
            infomax = infomax_base(infomax_path="contin_infomax/pretrained_infomax_32_stack1_earlystop_action_conti_nonexpert.ckpt",
                                   ndim=ndim,
                                   device=self.device,
                                   freeze_w=freeze_w)
            infomax2 = infomax_base(infomax_path="contin_infomax/pretrained_infomax_32_stack4_earlystop_action_conti_nonexpert.ckpt",
                                   ndim=ndim,
                                   device=self.device,
                                   freeze_w=freeze_w)
            self.actor = ActorPPO(infomax, fc, alp_bet_h).float().to(device)
            self.critic = Critic(infomax, v).float().to(device)
            self.critic_target = Critic(infomax2, v).float().to(device)
            print("\nPPO INFOMAX MODIFIED STARTED\n")
        elif rnn:

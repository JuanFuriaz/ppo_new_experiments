...
class PPO(object):
    def __init__(
        ...
        if self.lr_min > 0:
            self.scheduler = CosineAnnealingLR(self.optimizer, self.steps_opt, eta_min=self.lr_min)
        ...
        if vae:  # Variational Autoenconder
            vae = vae_base(vae_path=rl_path,
                           ...
                           device=self.device,
                           freeze_w=freeze_w)
            self.actor = ActorPPOVAE(vae, fc, alp_bet_h).float().to(device)
            ...
            self.actor = ActorPPO(raw_cnn, fc, alp_bet_h).float().to(device)
            self.critic = Critic(raw_cnn, v).float().to(device)
            print("\nPPO RAW-PIXEL STARTED\n")
        # Join optimization of actor and critic
        params_opt = list(self.actor.parameters()) + list(self.critic.parameters())
        self.optimizer = optim.Adam(params_opt, lr=self.lr)
    ...
    def train(self, buffer, batch_size=100):
        ...
        with torch.no_grad():  # Compute a vector with advantage terms
            ...
            if self.action_vec > 0:
                target_v = r + self.gamma * self.critic((s_, a_v[:, 3:]))
        ...
        for _ in range(self.ppo_epoch):
            ...
            for index in BatchSampler(SubsetRandomSampler(range(buffer.buffer_capacity)), batch_size, False):
                ...
                # Loss with Entropy
                loss = action_loss + self.value_l_sc * value_loss - self.entropy_l_sc * entropy
                ...
                if self.max_grad_norm:
                    nn.utils.clip_grad_norm_(list(self.actor.parameters()) + list(self.critic.parameters()),
                                             self.max_grad_norm)
                self.optimizer.step()
                self.loss_critic.append(value_loss)
                self.loss_actor.append(action_loss)
                self.loss_agent.append(loss)
                if self.lr_min > 0:
                    self.scheduler.step()
